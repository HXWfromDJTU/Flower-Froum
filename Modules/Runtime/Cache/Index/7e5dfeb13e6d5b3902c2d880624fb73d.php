		...
		<div id="page-essay" class="page page-essay">
			...
			<div class="content">
				...
				<div class="list-block media-list">
					<ul id="essayContainer">
						<!--<li>
							<a href='<?php echo U("index/essay_show");?>'>
								<div class="item-content">
									<div class="item-inner">
										<div class="item-title-row first-row">
											<img src="/hyltxt/View/Index/Public/img/icon.jpg">
											<p class="user-name">赖头头</p>
											<p class="post-time">19:23</p>
										</div>
										<div class="item-title-row second-row">
											<div class="item-title">雷杜德毕生的花卉作品</div>
										</div>
										<div class="item-title-row third-row">
											<p class="badge">花艺师</p>
											<p>阅读<span>455</span></p>
											<p>评论<span>124</span></p>
											<p>喜欢<span>665</span></p>
										</div>
									</div>
			...
			<script>
				$.loadEssay = function() {
					$.ajax({
						type: "post",
						url: "<?php echo U('media/loadEssay');?>",
						async: true,
						data: {
							/*keyword: keyword*/
						},
						success: function(d) {
							...
							if(data.code == "0000") {
								...
								data.data.forEach(function(arg, index) {
									tempStr.push('<li onclick="$.eaasyDetail('+arg.essayid+','+arg.authorid+')"><a><div class="item-content"><div class="item-inner">');
									tempStr.push('<div class="item-title-row first-row"><img src=' + arg.headimgsrc + '>');
									tempStr.push('<p class="user-name">' + arg.nickname + '</p><p class="post-time">' + arg.posttime.substring(5,16) + '</p></div>');
									tempStr.push('<div class="item-title-row second-row"><div class="item-title">' + arg.title + '</div></div>');
									tempStr.push('<div class="item-title-row third-row"><p class="badge">' + arg.tag + '</p><p>阅读<span>'+ arg.readnum+'</span></p><p>评论<span>3</span></p></div></div>');
									tempStr.push('<div class="item-media target-img"><img src=' + arg.img1 + '></div></div></a></li>');
								});
								$('#essayContainer').empty();	
								$('#essayContainer').append(tempStr.join(''));
							} else {
								$.toast("通讯异常");
							}
						}
				...
				$.loadEssay();
				$.eaasyDetail =function(essayID,authorID){
